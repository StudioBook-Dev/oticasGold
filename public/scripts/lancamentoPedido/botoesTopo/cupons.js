...
function abrirCupom() {
    // Busca os cupons disponíveis
    getCupons()
        .then(cupons => {
            // Cria a tabela HTML com os cupons
            let html = `
                <div class="selecionar-cupom-container">
                    <form id="form-selecionar-cupom">
                        <div class="acoes-modal acoes-modal-topo">
                            <button type="button" class="btn-modal-secundario btn-modal-secundario-cancel" onclick="fecharModalSecundario()">Cancelar</button>
                            <button type="button" class="btn-modal-secundario btn-modal-primario" onclick="aplicarCupomSelecionado()">Aplicar Cupom</button>
                        </div>
                        <table class="tabela-selecionar-cupom">
                            <thead>
                                <tr></tr>
                            </thead>
                            <tbody>`;
            ...
            if (cupons && cupons.length > 0) {
                // Adiciona cada cupom como uma linha na tabela
                cupons.forEach(cupom => {
                    // Formata o valor do cupom
                    let valorFormatado = '';
                ...
                // Mensagem para quando não há cupons disponíveis
                html += `
                    <tr>
                        <td colspan="3" class="sem-cupons">Nenhum cupom disponível</td>
                    </tr>`;
            }
            ...
            // Abre o modal secundário com a tabela de cupons
        ...
        .catch(error => {
            ...
            // Em caso de erro, mostra uma mensagem genérica
            abrirModalSecundario({
                titulo: 'Selecionar Cupom',
                conteudo: '<p>Erro ao carregar cupons. Tente novamente mais tarde.</p>'
            });
        });
}
